...
from search_spaces.tfe_ops import (relu, batch_normalization, conv2d,
                                   max_pool2d, fc_layer, dropout, flatten, add)
...
def genetic_stage(input_fn, node_fn, output_fn, h_connections, num_nodes):
    ...
    def substitution_fn(**dh):
        ...
        # There are no connections in the stage, perform single convolution
        # operation
        if sum(num_ins) == 0:
            return input_fn()
...
class SSF_Genetic(mo.SearchSpaceFactory):
    ...
    def __init__(self):
        mo.SearchSpaceFactory.__init__(
            # self, lambda: generate_search_space([3, 4, 5], [8, 16, 32],
            #                                     [5, 5, 5])
            self,
            lambda: generate_search_space([3, 4, 5], [64, 128, 256], [5, 5, 5]))
